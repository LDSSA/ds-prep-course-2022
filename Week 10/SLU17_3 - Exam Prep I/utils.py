...
class DataGenerator():
    def __init__(self):
        ...
        """
        self.data = [{"animal_name": "gull", "hair": 0, "feathers": "?", "eggs": 1, "milk": 0, "aquatic": "?", "toothed": 0, "fins": 0, "legs": 2, "class_type": "Bird"},
        ...
        self.data_replaced=[{k: v.replace('?', '1') if isinstance(v, str) else v
                         for k, v in animal.items()}
                         for animal in self.data]
        self.data_int=[{k: int(v) if isinstance(v, str) and v.isdigit() else v
                    for k, v in animal.items()}
                    for animal in self.data_replaced]
        self.data_lower=[{k: v.lower() if k == 'class_type' else v
                  for k, v in animal.items()}
                  for animal in self.data_int]
